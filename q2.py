...
def a_star(grid, start, goal, tie_breaking, callbacks=None):
...
def repeated_forward_astar(
    ...
    while current != goal:
        ...
        path, expanded = a_star(agent_grid, current, goal, tie_breaking, callbacks=cb)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        current_algo = "max_g"
                        show_astar_search(win, selected_maze, algo=current_algo, fps=60, step_delay_ms=0, save_path=args.save_vis_path)
